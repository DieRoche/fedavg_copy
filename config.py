...
def quantization_bits_type(value):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError("Quantization bits must be one of: none, 16, 8, 4.") from exc
    if bits not in (16, 8, 4):
        raise argparse.ArgumentTypeError("Quantization bits must be one of: none, 16, 8, 4.")
    return bits
...
def get_config():
    ...
    parser.add_argument(
        ...
        default=None,
        help="Post-sparsity quantization bits for client-to-server payload (none, 16, 8, or 4).",
    )
